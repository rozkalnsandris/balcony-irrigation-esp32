...
`START`, `turpini`, or equivalent continuation may carry safe source work from fresh canonical GitHub state through Ready. Documentation, native tests, source refactors and firmware source changes may proceed in one coherent batch, including branch, PR, CI/review and up to two scope-preserving corrections. Batch 2-5 closely related same-risk items when coherent. Merge remains explicit.
